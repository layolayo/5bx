...
class Bio5BXApp(tk.Tk):
    ...
    def show_dashboard(self):
        ...
        targets = s_targets[:4] + [c_targets[4]]
        ...
        for i, target in enumerate(targets):
            use_chart = s_chart if i < 4 else c_chart
            details = bx.get_exercise_detail(use_chart, i)
            ...
            tk.Button(row, text="📈", font=("Arial", 10), 
                      command=lambda idx=i, name=details['name'], chart=use_chart: self.show_exercise_history(chart, idx, name)).pack(side=tk.RIGHT)
    ...
    def show_badges_screen(self):
        ...
        badges = bx.get_earned_badges(s_chart, s_level, c_chart, c_level)
    ...
    def render_chart_grid(self):
        ...
        exercises = []
        for i in range(5):
            # Check which chart to use for names? Usually same names per chart, but Ex 5 differs.
            # We are viewing 'self.view_chart_idx'.
        ...
        for idx, r in enumerate(rows):
            ...
            # Level Label Highlighting (Highlight if EITHER is current?)
            # Let's use Strength as primary (Green), Cardio as secondary (Red) or Mixed?
            # User request: "current Cardio level differently... - at the moment all the same level are highlighted" (referring to old behavior)
            ...
            for i in range(5):
                ...
                # Logic: Ex 1-4 (i 0-3) use Strength; Ex 5 (i 4) use Cardio
                if i < 4:
                    if is_s_row: cell_bg = "#27ae60" # Strength Green
                else:
                    if is_c_row: cell_bg = "#c0392b" # Cardio Red (using correct red hex)
                
                tk.Label(self.chart_content, text=str(val), bg=cell_bg, fg="white", font=("Arial", 14)).grid(row=grid_row, column=i+1, padx=2, pady=2, sticky="nsew")
    ...
    def run_exercise_screen(self):
        ...
        # History Button
        # Resolve Chart ID for History Filter logic
        ...
        tk.Button(frame, text=f"📈 View History: {details['name']}", 
                 command=lambda: self.show_exercise_history(hist_chart_filter, idx, details['name'])).pack()
    ...
    def finish_workout(self):
        ...
        c_missed = 0
        if self.reps_achieved[4] < self.target_reps_list[4]: c_missed += 1
        ...
        # 4. Final Verdict
        status = f"Str: {s_status} / Car: {c_status}"
        ...
        # Check for Milestones (Strength Driven)
        age = self.calculate_age(self.user_data.get('dob', '2000-01-01'))
        milestones = bx.check_milestones(age, s_chart, s_level, s_new_c, s_new_l)
        
        # MILESTONE POPUP
        if milestones:
            m_top = tk.Toplevel(self)
            m_top.title("ACHIEVEMENT UNLOCKED!")
            m_top.geometry("600x500") # Taller for image
        ...
        self.db_add_history(self.user_id, f"{s_chart}/{c_chart}", f"{s_level}/{c_level}", status, avg_session_hr, max_session_hr, end_session_rmssd, self.reps_achieved, stats_json)
    ...
    def show_exercise_history(self, target_chart_id, target_idx, title_name):
        ...
        target_idx: 0-4 (The index in the routine).
        title_name: Display name for the window title.
        """
        records = self.db_get_history(self.user_id)
        # Sort by ID ascending for time
        # records.reverse() # db_get_history usually returns correct order? Let's check.
        # db_get_history selects all. Usage elsewhere sorts? Let's assume raw order is insertion (old->new) or vice versa.
        # Actually in show_history_screen it iterates.
        ...
        # print(f"DEBUG: Opening History for Chart {target_chart_id} Ex {target_idx} ({title_name})")
        ...
        for r in records:
            ...
            if str(eff_chart) != str(target_chart_id):
                 # print(f"  -> Skip: Chart Mismatch {eff_chart} != {target_chart_id}")
                 continue
        ...
        ax1.set_title(f"Reps Count: {full_title}", color="white")
