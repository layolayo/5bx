...
class SessionLogger:
    """
    Logger class ported from ant_gui.py to maintain CSV compatibility.
    """
...
class Bio5BXApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Bio-Adaptive 5BX Trainer v2")
        ...
        # Logging
        self.logger = None
        self.current_context_trend = "Idle" # Will hold Exercise Name for CSV 'Trend' column
    ...
    def load_user_session(self, name, profile_path):
        ...
        # Initialize Logger with username
    ...
    def start_workout(self):
        ...
        self.current_context_trend = "Starting Workout"
    ...
    def run_exercise_screen(self):
        ...
        self.current_context_trend = f"C{chart}-Ex {idx + 1}: {details['name']}"
    ...
    def input_results(self):
        ...
        self.current_context_trend = f"C{chart}-Finished Ex {idx+1} - Reporting"
    ...
    def sensor_loop(self):
        ...
        chart = self.user_data.get("current_chart", "?")
        level = self.user_data.get("current_level", "?")
        ...
        while self.workout_active:
            ...
            if self.logger:
                self.logger.log(
                    ...
                    raw_rr=raw_rr, 
                    state=state_str, 
                    trend=self.current_context_trend, 
                    status=zone_status, 
                    raw_hex=raw_hex
                )
            
            time.sleep(1) # Log at 1Hz roughly, aligning with UI update speed roughly
    ...
    def destroy(self):
        ...
        # Ensure log takes final flush if incomplete (though stop() should handle it)
        if self.logger:
            self.logger.stop()
