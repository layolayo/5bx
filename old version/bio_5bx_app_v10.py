...
class Bio5BXApp(tk.Tk):
    ...
    def db_add_history(self, user_id, chart, level, verdict, avg_hr, max_hr, rmssd, reps_list=None, stats_json=None, ex5_type="standard", ex5_duration=0):
        ...
        conn = sqlite3.connect(USER_DB_FILE)
        c = conn.cursor()
        c.execute("""INSERT INTO history (user_id, timestamp, chart, level, verdict, avg_hr, max_hr, end_rmssd, ex1, ex2, ex3, ex4, ex5, segment_stats, ex5_type, ex5_duration)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                     (user_id, ts, chart, level, verdict, int(avg_hr), int(max_hr), int(rmssd), 
                      reps_list[0], reps_list[1], reps_list[2], reps_list[3], reps_list[4], stats_json, ex5_type, ex5_duration))
        conn.commit()
        conn.close()
    ...
    def show_history_screen(self):
        ...
        for r in records:
            # Format Level: "Chart Grade" (e.g. 3 C-)
            c_raw = str(r['chart'])
            l_raw = str(r['level'])
            ...
            if "/" in c_raw:
                try:
                    ...
                    s_c = c_parts[0]
                    c_c = c_parts[1] if len(c_parts)>1 else s_c
                    s_l = l_parts[0]
                    c_l = l_parts[1] if len(l_parts)>1 else s_l
                    
                    s_disp = bx.get_level_display(s_l)
                    c_disp = bx.get_level_display(c_l)
                    full_level = f"{s_c} {s_disp} / {c_c} {c_disp}"
                except:
                    full_level = f"{c_raw}-{l_raw}"
            else:
                lvl_disp = bx.get_level_display(l_raw)
                full_level = f"{c_raw} {lvl_disp}"
            ...
            if 'ex1' in r.keys() and r['ex1'] is not None:
                ...
                if ex5_type == 'standard' or not ex5_type:
                    reps_str += f"-{r['ex5']}"
                else:
