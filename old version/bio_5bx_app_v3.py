...
class SessionLogger:
    ...
    def start(self):
        ...
        self.writer.writerow(["Timestamp", "HR_BPM", "RMSSD_MS", "Raw_RR_MS", "State", "Trend", "Status", "Raw_Packet_Hex"])
...
class Bio5BXApp(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1000x900") # Taller for report
        ...
        self.session_metrics = [] # List of dicts: [{'name':'Situp', 'hr':[], 'rmssd':[]}, ...]
        self.logger = None
    ...
    def _init_db(self):
        try:
            ...
            # --- MIGRATION: Check if ex1..ex5 exist (for v2->v3 upgrade) ---
    ...
    def show_dashboard(self):
        ...
        lbl_stats = ttk.Label(stats_frame, text=f"❤️ RHR: {rhr}  |  ⚡ HRV: {rmssd}ms  |  🔋 Rec: {rec}", foreground="#1abc9c", font=("Arial", 11))
        ...
        for i, target in enumerate(targets):
            ...
            # Graph Button
            # Use lambda with default arg to capture 'i' and 'name' correctly
            btn_hist = tk.Button(row, text="📈", font=("Arial", 10), 
                                 command=lambda idx=i, name=details['name']: self.show_exercise_history(name, idx))
    ...
    def show_history_screen(self):
        ...
        for r in records:
            ...
            # Format Reps: check if keys exist (migration might fill with 0/None)
            reps_str = ""
    ...
    def start_workout(self):
        self.current_exercise_idx = 0
        self.session_metrics = [] # Reset Metrics
        self.reps_achieved = []
    ...
    def run_exercise_screen(self):
        ...
        # Initialize Metrics for this exercise
    ...
    def sensor_loop(self):
        while self.workout_active:
            ...
            hr = data['bpm']
            rmssd = data['rmssd']
            rr = 0 # Placeholder if not exposed
            ...
            if hr > (max_hr * 0.9): status = "TOO HIGH"
            elif hr < (max_hr * 0.6) and self.current_exercise_idx == 4: status = "PUSH"
            
            if hr > 0:
                # Log to UI Lists (Analysis)
                ...
                if self.logger:
                    ...
                    # Safe retrieval for race condition (thread starts before UI inits metrics)
                    if self.current_exercise_idx < len(self.session_metrics):
                        trend_name = self.session_metrics[self.current_exercise_idx]['name']
                    else:
                        trend_name = "Starting..."
                        
                    self.logger.log(hr, rmssd, rr, state_str, f"C{chart}-Ex {self.current_exercise_idx+1}: {trend_name}", status, "")

                # UI Update
                try:
                    self.lbl_hr.config(text=f"♥ {hr} BPM")
                    if status == "TOO HIGH": self.lbl_advice.config(text="⚠️ TOO HIGH!", foreground="red")
                    elif status == "PUSH": self.lbl_advice.config(text="⚡ PUSH!", foreground="yellow")
                    else: self.lbl_advice.config(text="✅ Zone OK", foreground="#2ecc71")
                except: pass
            time.sleep(1)
    ...
    def generate_detailed_report(self):
        ...
        for i, metric in enumerate(self.session_metrics):
            ...
            if avg_hrv < 10 and "Back Arch" not in name:
                status += " / HIGH STRESS"
                total_stress_count += 1
                
            # Reps Info
            try:
                rep_done = self.reps_achieved[i]
                rep_targ = self.target_reps_list[i]
                rep_str = f"Reps: {rep_done}/{rep_targ}"
            except:
                rep_str = "Reps: ?/?"

            line = f"{name}: {rep_str} | Avg HR {int(avg_hr)} | Max {max_hr} | HRV {hrv_str}"
    ...
    def finish_workout(self):
        ...
        all_rmssd = [r for i, m in enumerate(self.session_metrics) for r in m['rmssd'] if i != 2] # Skip Back Arch
        ...
        # LOGIC TREE
        if max_hr > (self.bio_profile.max_hr * 0.95) or physio_warnings > 1:
        ...
        self.db_add_history(self.user_id, chart, level, db_verdict, avg_hr, max_hr, end_rmssd, self.reps_achieved, stats_json)
